...
Now we have a function that simulate the coevolution of a victim and an exploiter species under any type of matrix of effects between alleles. Let's start simulating coevolution under a gene-for-gene mechanism. As we studied in the lecture,  for this mechanism the matrix takes the following form:
...
\right)\end{array}
$$
Thus, we need to set up this matrix first, to pass it as an argument (the matrix $A$ in the code) to our function.


## Simulating coevolution of mutualistic interactions

In the previous section, we simulated coevolution of one victim and exploiter species under two different mechanisms determining the outcome of the interaction, gene-for-gene or matching allele. Here, we will slightly modify this coevolutionary model to encompass mutualistic interactions. Mutualisms are interactions that increase the fitness of individuals.
